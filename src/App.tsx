...
import { ArrowLeft, ArrowUpRight, Bot, Check, ChevronRight, CircleHelp, Clock3, Flag, Focus, Layers3, Maximize2, Minimize2, Music2, Pause, Play, RotateCcw, RotateCw, UsersRound, Volume2, VolumeX, X } from 'lucide-react';
    ...
    <main className="workspace">
      ...
      <aside className="game-panel" aria-label="对局设置和棋谱">
        <section className="settings-section"><div className="section-title"><h2>对局设置</h2><span>GAME MODE</span></div>
          ...
          {mode === 'npc' ? <><div className="setting-label"><span>选择棋友</span><span className="character-picker-caption">以棋会友</span></div><div className="character-options" role="group" aria-label="NPC 棋友">{characters.map((npc) => <button key={npc.id} className={`character-option ${difficulty === npc.difficulty ? 'active' : ''}`} data-character={npc.id} aria-label={`与${npc.name}对弈`} aria-pressed={difficulty === npc.difficulty} onClick={() => { if (difficulty !== npc.difficulty) requestRestart(mode, npc.difficulty); }}><span className="character-portrait"><img src={npc.portrait} alt="" />{difficulty === npc.difficulty && <span className="character-check"><Check size={11} /></span>}</span><strong>{npc.name}</strong><small>{npc.title}</small></button>)}</div><div className="character-intro"><p>{character.invitation}</p><span>{character.style}</span></div></> : <div className="local-description"><UsersRound size={24} /><p>与身边的朋友切磋<span>同一设备，红黑双方轮流落子</span></p></div>}
