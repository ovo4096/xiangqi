...
import { ArrowLeft, ArrowUpRight, Bot, Check, ChevronRight, CircleHelp, Clock3, Flag, Focus, Layers3, Maximize2, Minimize2, RotateCcw, RotateCw, UsersRound, Volume2, VolumeX, X } from 'lucide-react';
import { BoardScene } from './scene/BoardScene';
import { applyMove, describeMove, getGameResult, initialBoard, isInCheck, legalMoves, pieceLabel, type Piece, type Side } from './game/engine';
...
const levels: { id: Difficulty; name: string; caption: string }[] = [
  { id: 'easy', name: '入门', caption: '轻松切磋，熟悉棋路' },
  { id: 'medium', name: '进阶', caption: '攻守之间，见招拆招' },
  { id: 'hard', name: '高手', caption: '步步为营，认真应战' },
];
...
function Modal({ title, children, onClose, className = '' }: { title: string; children: ReactNode; onClose: () => void; className?: string }) {
  ...
  return <dialog className={`modal ${className}`} ref={dialog} onCancel={(e) => { e.preventDefault(); onClose(); }} onClick={(e) => { if (e.target === dialog.current) onClose(); }}>
...
function App() {
  ...
  const check = !result && isInCheck(game.board, game.turn);
  ...
  const move = useCallback((pieceId: string, to: Coord) => {
    ...
    setGame({ board: applyMove(game.board, pieceId, to), turn: game.turn === 'red' ? 'black' : 'red', history: [...game.history, entry] });
    setSelectedId(null); setAiFailed(false); playSound(!!captured);
  }, [game, result, playSound]);
  ...
  const status = result ? `${result.winner ? sideName(result.winner) + '获胜' : '和棋'}` : aiFailed ? '棋友暂时停顿' : thinking ? '棋友思考中' : check ? `${sideName(game.turn)}被将军` : `${sideName(game.turn)}行棋`;
  ...
  return <div className="app-shell">
      ...
      <section className="play-area" aria-label="中国象棋对弈区">
        ...
        <div className="board-stage">
          <div className={`player-seat opponent ${game.turn === 'black' && !result ? 'seat-active' : ''}`}>
            <span className="piece-avatar black-avatar">将</span><div><strong>{mode === 'npc' ? '弈境棋友' : '黑方棋手'}{mode === 'npc' && <span className="npc-tag">NPC</span>}</strong><span className="seat-caption">{mode === 'npc' ? levels.find((l) => l.id === difficulty)?.name + '棋力' : '本地双人'}<span className="seat-dot">·</span>执黑</span></div>
        ...
        <div className="board-footer"><span><span className="mouse-icon" />拖动旋转<span className="footer-dot">·</span>滚轮缩放<span className="footer-dot">·</span>点击落子</span><span><span className="small-circle" />落子无声，亦有回响</span></div>
      </section>

      <aside className="game-panel" aria-label="对局设置和棋谱">
        <section className="settings-section"><div className="section-title"><h2>对局设置</h2><span>GAME MODE</span></div>
          <div className="mode-tabs" role="group" aria-label="对战模式"><button className={mode === 'npc' ? 'active' : ''} aria-pressed={mode === 'npc'} onClick={() => { if (mode !== 'npc') requestRestart('npc'); }}><Bot size={18} />人机对战</button><button className={mode === 'local' ? 'active' : ''} aria-pressed={mode === 'local'} onClick={() => { if (mode !== 'local') requestRestart('local'); }}><UsersRound size={17} />双人对战</button></div>
          {mode === 'npc' ? <><div className="setting-label"><span>棋友难度</span><span className="difficulty-bars"><i /><i className={difficulty !== 'easy' ? 'lit' : ''} /><i className={difficulty === 'hard' ? 'lit' : ''} /></span></div><div className="difficulty-options" role="group" aria-label="NPC 难度">{levels.map((level) => <button key={level.id} className={difficulty === level.id ? 'active' : ''} aria-pressed={difficulty === level.id} onClick={() => { if (difficulty !== level.id) requestRestart(mode, level.id); }}>{level.name}{difficulty === level.id && <span className="difficulty-check"><Check size={10} /></span>}</button>)}</div><p className="setting-note">{levels.find((l) => l.id === difficulty)?.caption}</p></> : <div className="local-description"><UsersRound size={24} /><p>与身边的朋友切磋<span>同一设备，红黑双方轮流落子</span></p></div>}
          <button className="primary-button new-game" onClick={() => requestRestart()}><RotateCw size={17} /><span>开始新对局</span><ArrowUpRight size={17} /></button>
        </section>

        <section className="status-section" aria-live="polite"><div className="status-top"><span className={`turn-light ${game.turn} ${thinking ? 'pulsing' : ''}`} /><strong>{status}</strong><span className="round-label">第 {Math.floor(game.history.length / 2) + 1} 回合</span></div><p>{result ? result.reason : aiFailed ? '可重试，或悔棋重新落子' : thinking ? '观棋不语，静候佳着' : check ? '请先解除将军，保护主帅' : selected ? `已选择「${pieceLabel(selected)}」，点击标记位置落子` : '选择一枚棋子，开启下一步'}</p>{aiFailed && <button className="text-button" onClick={() => { setAiFailed(false); setRetry((n) => n + 1); }}>重新思考 <RotateCw size={14} /></button>}</section>
        ...
        <section className="history-section"><div className="section-title"><h2>走棋记录</h2><span className="move-count">{game.history.length.toString().padStart(2, '0')} 步</span></div><div className="history-columns"><span>回合</span><span><i className="red-dot" />红方</span><span><i className="black-dot" />黑方</span></div>
          <div className="move-history" ref={historyEl}>{game.history.length === 0 ? <div className="empty-history"><span className="empty-chess">弈</span><p>棋局初开，静候落子</p><span>每一步，皆有可能</span></div> : Array.from({ length: Math.ceil(game.history.length / 2) }, (_, i) => <div className="history-row" key={i}><span>{(i + 1).toString().padStart(2, '0')}</span><span className={i * 2 === game.history.length - 1 ? 'latest' : ''}>{game.history[i * 2]?.label}</span><span className={i * 2 + 1 === game.history.length - 1 ? 'latest' : ''}>{game.history[i * 2 + 1]?.label || <span className="waiting-move">—</span>}</span></div>)}</div>
        </section>
        <section className="captures-section" aria-label="吃子记录"><div><span>红方吃子</span><div>{capturedBy('red').length ? capturedBy('red').map((p) => <b key={p.id} className="captured-piece black-capture" title={pieceLabel(p)}>{pieceLabel(p)}</b>) : <i>—</i>}</div></div><div><span>黑方吃子</span><div>{capturedBy('black').length ? capturedBy('black').map((p) => <b key={p.id} className="captured-piece red-capture" title={pieceLabel(p)}>{pieceLabel(p)}</b>) : <i>—</i>}</div></div></section>
    ...
    {help && <Modal title="对弈指南" onClose={() => setHelp(false)} className="help-modal"><p className="modal-intro">一方棋盘，红黑两军。将死或困毙对方，即可获胜。</p><div className="guide-items"><div><span>01</span><div><h3>选择你的对手</h3><p>人机对战由你执红先行，NPC 执黑。双人对战在同一设备上交替操作。</p></div></div><div><span>02</span><div><h3>选棋，再落子</h3><p>点击己方棋子查看合法落点，再点击标记的位置。再次点击所选棋子可取消选择。</p></div></div><div><span>03</span><div><h3>换个角度看棋局</h3><p>拖动棋盘旋转视角，滚轮或双指缩放。「俯视」切换为平直视角，「重置视角」回到初始位置。</p></div></div></div><div className="rules-grid"><p><b>车</b>横直行走，不可越子</p><p><b>马</b>走日字，注意蹩马腿</p><p><b>象</b>走田字，不得过河</p><p><b>士</b>斜行一步，不离九宫</p><p><b>将</b>九宫内行走，将帅不照面</p><p><b>炮</b>走如车，隔一子吃子</p><p><b>兵</b>向前一步，过河后可横行</p></div><p className="keyboard-help">键盘操作：聚焦棋盘后，方向键移动光标，回车或空格选棋 / 落子，Esc 取消。支持将军、将死与困毙判定；赛事中的长将、长捉和自然限着裁定暂未实现。</p><button className="primary-button" onClick={() => setHelp(false)}>入局对弈 <ChevronRight size={16} /></button></Modal>}
