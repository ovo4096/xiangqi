...
- 四位棋友共有 **144 句离线角色语音**，覆盖开场、吃子、被吃、痛失大子、思考、悔棋和胜负。同类台词轮换播放，避免紧邻重复；语音可独立关闭、调节音量和重播，说话时背景音乐自动降低。
...
更新 `package.json` 与 `package-lock.json` 中的版本号，提交修改后推送对应的 `v版本号` 标签。例如发布 `1.3.1`：
...
- `public/music/`：离线背景音乐。
- `public/voices/`：144 句离线角色语音和校验清单。
